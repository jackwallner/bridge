...
struct FlashcardDrillView: View {
    ...
    @AppStorage("bridge.hasSwipedDeck") private var hasSwipedDeck = false
...
struct FlipCardFace: View {
    ...
    private var front: some View {
        BridgeCardFace(accent: accent, eyebrow: "BRIDGE TRAINER") {
            VStack(spacing: 18) {
                Spacer(minLength: 0)
                ...
                if !card.frontCards.isEmpty {
                    BridgeHandView(cards: card.frontCards, cardWidth: 52)
                }
    ...
    private var back: some View {
        BridgeCardFace(accent: accent, eyebrow: "THE CALL") {
            VStack(spacing: 14) {
                if let verdict {
...
/// The bridgeong-card chrome both faces share: ivory surface, double frame like
/// the printed card, corner pips, and a faint tile-glyph watermark so the
/// open space feels designed instead of empty.
private struct BridgeCardFace<Content: View>: View {
    let accent: Color
    let eyebrow: String

...
/// Shared question scaffolding: prompt + cards + choices + explanation, the
...
struct QuestionPager<Choices: View>: View {
    ...
    var body: some View {
        // Centring, not top-aligned: on a 13-inch iPad a four-choice question
        ...
        CenteringScrollView {
            ...
            VStack(spacing: 20) {
                ...
                Text(prompt)
                    ...
                    .padding(.top, 8)
                if !cards.isEmpty {
                    BridgeHandView(cards: cards, cardWidth: 44)
                }
                choices()
                if answered {
